...
// button click event to start renderResults function
function renderButtons() {
    let btns = $('#citiesView');
    btns.empty();

    // for loop to create newBtn 
    for (i=0; i<cities.length; i++) {
        let newBtn = $('<button>');
        newBtn.text(cities[i]);
        newBtn.attr('cityName', cities[i]);
        newBtn.attr('class', cities[i]);
        newBtn.css('width', '100%');
        btns.append(newBtn);                
      }    
    }; 

// Onclick function to start the query for the city entered
// create 2 onclick events to start function make a function
    ...
    function startQuery() {        
        ...
        let queryURL ='https://api.openweathermap.org/data/2.5/weather?q=' + cityName + '&appid=6d38855c9713d8c7df4486fb228ef4f5&units=imperial';
        // query openweathermap 5 day forcast
        let queryURL5Day ='https://api.openweathermap.org/data/2.5/forecast?q='+ cityName +',us&mode=json&appid=6d38855c9713d8c7df4486fb228ef4f5&units=imperial';
        renderButtons();
        ...
        .then(function(response5Day) {
            // clear all elements so new data repopulates the field properly
            $('#day1Date').empty();
            $('#day1Icon').empty();
            $('#day1Temp').empty();
            $('#day1Humidity').empty();
        ...
        .then(function(response) {
            ...
            // variables from weather queryURL to add to queryURLuv to get UV Index
            let lat = response.coord.lat;
            let lon = response.coord.lon;            
        
            // queryURL for UV Index
            let queryURLuv ='https://api.openweathermap.org/data/2.5/uvi?appid=6d38855c9713d8c7df4486fb228ef4f5&lat=' + lat + '&lon=' + lon;
